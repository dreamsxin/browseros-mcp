...
| `--name` | browser-control-mcp | MCP server name |
...
`windowId`, active-tab state, windows, and tab groups. See
`docs/chrome-extension-bridge.md` for the design and setup details.
...
For command-line loading, use an isolated Chrome profile so Chrome does not
reuse an existing process and ignore `--load-extension`:
...
| `windows` | Manage browser windows | ✅ Full | ✅ With bridge |
...
|--------|-------------|-----------|
| `browser-automation` | Browser observe-act-verify guidance for browser tasks. | `task` (optional string) |
...
If `--mcp-url` or `BROWSER_CONTROL_MCP_URL` is not provided, the script prompts
for the MCP URL first and defaults to `http://127.0.0.1:3000/mcp`.
